...
package ExternalAmbient
  model AmbientData
    ...
    parameter Real latitude =    35.8083;
     ...
     replaceable model AmbientConditionsSource =
        StephanieLibrary.AM.AmbientConditions_Table
                                                constrainedby
      StephanieLibrary.AM.AmbientConditionsSourcePartialModel
                                                annotation(choicesAllMatching=true);

  //StephanieLibrary.AM.FixedAmbient ambient;
   inner StephanieLibrary.AM.Ambient  ambient( initialMonth=initialMonth,
  ...
  //model for theta, with input parameters trackingAxisOrientation and trackingAxisSlope. the rest of the variables are taken from ambien (sunposition, ecc ecc)
              Bricker.Components.ExternalAmbient.Theta ThetaModel( trackingAxisOrientation=trackingAxisOrientation,trackingAxisSlope=trackingAxisSlope);
...
- Theta"),Text(
  ...
  import StephanieLibrary.Constants;
  import StephanieLibrary.Utilities.Functions;
  import StephanieLibrary.AM;

  //Models
    outer StephanieLibrary.AM.Ambient ambient;
  //Parameters
    parameter Real trackingAxisOrientation(min = 0, max = halfPi) = 0
      "Tracking axis orientation (rad) with 0 for a North-South axis and Pi/2 for an East-West axis.";
    parameter Real trackingAxisSlope(min = 0, max = halfPi) = 0
  ...
  //Variables
    ...
    StephanieLibrary.AM.Ambient.SunPosition sun;
    constant Real halfPi = Constants.pi/2;
