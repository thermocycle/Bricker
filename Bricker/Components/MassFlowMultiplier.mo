...
// missing: add initialization with temperature for hstart
      parameter Medium.SpecificEnthalpy hstart_prova=Medium.specificEnthalpy_pTX(pstart,500,fill(0,0))
...
/***************  VARIABLES ******************/
  ...
  Modelica.SIunits.MassFlowRate M_dot(start=Mdotnom); //internal mass flow for each parallel line, always < M_dot_total
  Modelica.SIunits.MassFlowRate M_dot_total; //total flow given to the external system
...
/* pressures */
  ...
  InFlow.p = p;
/*Mass Flow*/
M_dot_total  = M_dot * Np; //mass flow ficticious multiplier
  M_dot = InFlow.m_flow; //InFlow si considera il flusso visto all'interno
  OutFlow.m_flow = -M_dot_total; //OutFlow è il connettore che va messo sull'esterno, e che riceve la moltiplicazione di portata
...
equation
  ...
  annotation (Diagram(coordinateSystem(preserveAspectRatio=true,  extent={{-100,
          ...
          textString="Mass Flow Multiplier")}),
