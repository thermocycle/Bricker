...
model Test_SF_BiomassSystem_ORC_Hx_II_Simone_V3_0_finale
...
ThermoCycle.Components.Units.Solar.SolarField_Soltigua_Inc        solarCollector_Soltigua(
 ...
 BrickerISES.Components.Valve_lin ValveSF1(
  ...
  Modelica.Blocks.Sources.Constant const5(k=333)
    annotation (Placement(transformation(extent={{-42,144},{-32,154}})));
  Modelica.Fluid.Sensors.Temperature Temp_HEX_water_outlet(redeclare package
      Medium = ThermoCycle.Media.Water)
    annotation (Placement(transformation(extent={{234,148},{212,162}})));
  Modelica.Fluid.Sensors.Temperature Temp_HEX_water_inlet(redeclare package
      Medium = ThermoCycle.Media.Water)
    annotation (Placement(transformation(extent={{246,98},{224,112}})));
  Modelica.Blocks.Sources.TimeTable timeTable1(table=[0,333; 2000,333; 2001,330;
        8000,330; 8001,330; 9000,330])
    annotation (Placement(transformation(extent={{250,48},{236,62}})));
...
equation
  ...
  connect(Temp_HEX_water_outlet.port, sinkP.flangeB) annotation (Line(
      points={{223,148},{224,148},{224,134},{286,134},{286,137.6}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(pID2.PV, Temp_HEX_water_outlet.T) annotation (Line(
      points={{12,113.6},{-60,113.6},{-60,170},{188,170},{188,155},{215.3,155}},
      color={0,0,127},
      smooth=Smooth.None));

  connect(pID2.CS, ValveSF1.cmd) annotation (Line(
      points={{22.3,116},{38,116},{38,100},{54,100}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(const5.y, pID2.SP) annotation (Line(
      points={{-31.5,149},{-2,149},{-2,118.4},{12,118.4}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(Temp_HEX_water_inlet.port, hx1DInc1.inlet_fl1) annotation (Line(
      points={{235,98},{228,98},{228,88},{204.923,88},{204.923,97.3462}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(timeTable1.y, sourceMdot.in_T) annotation (Line(
      points={{235.3,55},{226.65,55},{226.65,53.8},{218,53.8}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-260,
            ...
            -200},{240,160}})));
end Test_SF_BiomassSystem_ORC_Hx_II_Simone_V3_0_finale;
