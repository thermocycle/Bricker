...
model Test_SF_BiomassSystem_ORC_Hx_II_Simone_V3_8_prova
...
ThermoCycle.Components.Units.Solar.SolarField_Soltigua_Inc        solarCollector_Soltigua(
  ...
  Modelica.Fluid.Sensors.Temperature TempSfSysOut_SF(redeclare package Medium
      = ThermoCycle.Media.Incompressible.IncompressibleTables.TherminolSP)
    annotation (Placement(transformation(extent={{-8,-70},{20,-52}})));
 ...
 BrickerISES.Components.Valve_lin Valve_HEX(
  ...
  Modelica.Blocks.Sources.Constant const6(k=518)
    annotation (Placement(transformation(extent={{56,82},{46,92}})));
 ...
 BrickerISES.Components.Valve_lin ValveSF1(
    ...
    Xopen=0.99) annotation (Placement(transformation(
        ...
        rotation=0,
        origin={-26,42})));
  Modelica.Fluid.Sensors.MassFlowRate Mflow_BmRecirculation1(
                                                            redeclare package
      Medium =
        ThermoCycle.Media.Incompressible.IncompressibleTables.TherminolSP)
    annotation (Placement(transformation(
  ...
  ThermoCycle.Components.Units.Tanks.OpenTank Tank2(
    ...
    Tstart=477.15,
    pstart=450000)
    annotation (Placement(transformation(extent={{32,36},{42,48}})));
...
equation
  ...
  connect(TempSfSysOut_SF.port, massFlowRate3.port_a) annotation (Line(
      points={{6,-70},{2,-70},{2,-76},{9,-76},{9,-77}},
      color={0,127,255},
      smooth=Smooth.None));
  ...
  connect(T_Inlet_orc.T, pID4.PV) annotation (Line(
      points={{39.6,64},{28,64},{28,57.6},{18,57.6}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(const6.y, pID4.SP) annotation (Line(
      points={{45.5,87},{24,87},{24,62},{22,62},{22,62.4},{18,62.4}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(Mflow_BmRecirculation.port_a, Mflow_Bm_out.port_b) annotation (Line(
      points={{-65,18},{-65,38.5},{-92,38.5}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(ValveSF1.InFlow, Mflow_Bm_out.port_b) annotation (Line(
      points={{-35,42},{-64,42},{-64,38.5},{-92,38.5}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(Tank1.OutFlow, massFlowRate_totBM.port_a) annotation (Line(
      points={{65.8,-76.4},{78,-76.4},{78,-33},{65,-33}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(pID4.CS, ValveSF1.cmd) annotation (Line(
      points={{7.7,60},{-26,60},{-26,50}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(Mflow_BmRecirculation.port_b, pump.InFlow) annotation (Line(
      points={{-65,-6},{-66,-6},{-66,-33.5},{-106.8,-33.5}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(Mflow_BmRecirculation1.port_a, massFlowRate_totBM.port_b) annotation (
     Line(
      points={{28,-26},{28,-33},{51,-33}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(ValveSF1.OutFlow, Tank2.InFlow) annotation (Line(
      points={{-17,42},{2.5,42},{2.5,36.96},{32.1,36.96}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(Tank2.OutFlow, massFlowRate_mix.port_a) annotation (Line(
      points={{41.9,36.96},{45.95,36.96},{45.95,40.5},{62,40.5}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(DpSF2.InFlow, Mflow_BmRecirculation1.port_b) annotation (Line(
      points={{28,8.8},{28,-10}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(DpSF2.OutFlow, Tank2.InFlow) annotation (Line(
      points={{28,23.2},{28,36.96},{32.1,36.96}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(massFlowRate_totBM.port_b, DpSF3.InFlow) annotation (Line(
      points={{51,-33},{-6.8,-33},{-6.8,-28}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(DpSF3.OutFlow, pump.InFlow) annotation (Line(
      points={{-21.2,-28},{-70,-34},{-106.8,-33.5}},
      color={0,0,255},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-260,
            ...
            -200},{440,160}})));
end Test_SF_BiomassSystem_ORC_Hx_II_Simone_V3_8_prova;
