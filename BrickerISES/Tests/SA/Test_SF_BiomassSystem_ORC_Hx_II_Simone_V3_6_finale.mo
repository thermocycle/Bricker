...
model Test_SF_BiomassSystem_ORC_Hx_II_Simone_V3_6_finale
...
ThermoCycle.Components.Units.Solar.SolarField_Soltigua_Inc        solarCollector_Soltigua(
  ...
  Modelica.Blocks.Sources.TimeTable timeTable(table=[0,0; 1000,0; 1001,400;
        3000,400; 3001,550; 5000,550; 5001,550; 6000,550; 6001,300; 9000,300;
        9001,0; 10000,0])
 ...
 BrickerISES.Components.Valve_lin ValveSF2(
    ...
    UseNom=false,
    Afull=9.5e-03,
    DELTAp_nom=1000) annotation (Placement(transformation(
        extent={{-10,-10},{10,10}},
        rotation=90,
        origin={12,6})));
  ...
  Modelica.Blocks.Sources.Constant const6(k=518)
    annotation (Placement(transformation(extent={{32,74},{22,84}})));
  ...
  Modelica.Fluid.Sensors.Temperature TempBM_in2(redeclare package Medium =
        ThermoCycle.Media.Incompressible.IncompressibleTables.TherminolSP)
    annotation (Placement(transformation(extent={{-94,78},{-66,96}})));
  Modelica.Fluid.Sensors.Temperature TempBM_out2(redeclare package Medium =
        ThermoCycle.Media.Incompressible.IncompressibleTables.TherminolSP)
    annotation (Placement(transformation(extent={{-28,78},{0,96}})));
  Modelica.Fluid.Sensors.Temperature TempBM_out3(redeclare package Medium =
        ThermoCycle.Media.Incompressible.IncompressibleTables.TherminolSP)
    annotation (Placement(transformation(extent={{34,-108},{62,-126}})));
equation
  ...
  connect(Tank1.OutFlow, massFlowRate_totBM.port_a) annotation (Line(
      points={{65.8,-76.4},{80,-76.4},{80,-33},{63,-33}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(ValveSF2.InFlow, massFlowRate_in_BM2.port_a) annotation (Line(
      points={{12,-3},{12,-19.5},{-5,-19.5}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(massFlowRate_totBM.port_b, massFlowRate_in_BM2.port_a) annotation (
      Line(
      points={{49,-33},{15.5,-33},{15.5,-19.5},{-5,-19.5}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(TempBM_out2.port, DpSF2.InFlow) annotation (Line(
      points={{-14,78},{-32,78},{-32,36},{-23.4,36}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(Mflow_HEX_oil.port_b, MflowToSf.port_a) annotation (Line(
      points={{130,16},{130,6},{86,6},{86,-174},{-4,-174.6},{-4,-174},{-34,-174}},
      color={0,127,255},
      smooth=Smooth.None));

  connect(TempBM_out3.port, ValveSF2.InFlow) annotation (Line(
      points={{48,-108},{48,-56},{12,-56},{12,-3}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(Mflow_BmRecirculation.port_b, pump.InFlow) annotation (Line(
      points={{-57,-8},{-82,-8},{-82,-33.5},{-106.8,-33.5}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(TempBM_in2.port, massFlowRate_in_BM2.port_b) annotation (Line(
      points={{-80,78},{-80,30},{-24,30},{-24,-19.5}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(massFlowRate_in_BM2.port_b, pump.InFlow) annotation (Line(
      points={{-24,-19.5},{-54,-19.5},{-54,-34},{-84,-34},{-106.8,-33.5}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(const6.y, pID3.SP) annotation (Line(
      points={{21.5,79},{21.5,70.5},{12,70.5},{12,60.4}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(T_Inlet_orc.T, pID3.PV) annotation (Line(
      points={{39.6,64},{26,64},{26,55.6},{12,55.6}},
      color={0,0,127},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-260,
            ...
            -200},{440,160}})));
end Test_SF_BiomassSystem_ORC_Hx_II_Simone_V3_6_finale;
