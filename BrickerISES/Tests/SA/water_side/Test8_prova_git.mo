...
model Test8_prova_git
  ...
  Modelica.Fluid.Sensors.MassFlowRate Mflow_HEX_water(redeclare package Medium
      = ThermoCycle.Media.Incompressible.IncompressibleTables.TherminolSP)
        ...
        rotation=0,
        origin={150,143})));
  Modelica.Blocks.Sources.Constant Qeva1(k=1.5)
    annotation (Placement(transformation(extent={{-5,-5},{5,5}},
        rotation=0,
        origin={53,149})));
  ...
  ThermoCycle.Components.Units.ControlSystems.PID pID_HEX(
    ...
    CSstart=225,
    CSmax=225 + 273,
    CSmin=69 + 273) annotation (Placement(transformation(
        extent={{9.5,7.5},{-9.5,-7.5}},
        rotation=180,
        origin={25.5,189.5})));
  ...
  BrickerISES.Components.Control.WatTBa watTBa
    annotation (Placement(transformation(extent={{426,22},{370,82}})));
  ...
  Modelica.Blocks.Logical.Hysteresis hysteresis(uLow=59.9 + 273, uHigh=60.3
         + 273)
    annotation (Placement(transformation(extent={{318,202},{338,222}})));
  Modelica.Blocks.Math.BooleanToReal booleanToReal(realTrue=-1, realFalse=0)
    annotation (Placement(transformation(extent={{368,200},{388,220}})));
  Modelica.Blocks.Sources.TimeTable timeTable1(table=[0,-387000; 20000,-387000;
    ...
    annotation (Placement(transformation(extent={{392,158},{378,172}})));
equation
  ...
  connect(HxOilWater.OutFlowSs, sinkP.flangeB) annotation (Line(
      points={{190.98,96.8},{108,96.8},{108,86.4}},
      color={0,0,255},
      smooth=Smooth.None));
  ...
  connect(Cooling_tower.inlet_fl2, Temp_out_cooling_tower.port) annotation (
      Line(
      points={{323.96,12},{326,12},{326,-10},{378,-10}},
      color={0,0,255},
      smooth=Smooth.None));
  ...
  connect(const1.y, pID_HEX.SP) annotation (Line(
      points={{-17.5,211},{-3.75,211},{-3.75,192.5},{16,192.5}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(watTBa.Qres, Cooling_tower.u) annotation (Line(
      points={{368.88,56.2},{348,56.2},{348,29},{333.2,29}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(watTBa.Qhx, HxOilWater.Q_hx) annotation (Line(
      points={{428.24,29.2},{436,29.2},{436,-106},{168,-106},{168,126},{
          196.04,126},{196.04,125.24}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(Qcond.y, watTBa.QORC) annotation (Line(
      points={{-39,97.5},{-39,72},{4,72},{4,-64},{486,-64},{486,52},{428.24,
          52}},
      color={0,0,127},
      smooth=Smooth.None));
  ...
  connect(booleanToReal.y, watTBa.ControlSignal) annotation (Line(
      points={{389,210},{403.04,210},{403.04,80.8}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(hysteresis.y, booleanToReal.u) annotation (Line(
      points={{339,212},{352,212},{352,210},{366,210}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(timeTable1.y, Ad_chiller.u) annotation (Line(
      points={{377.3,165},{348,165},{348,95},{331.2,95}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(watTBa.Qbuilding, timeTable1.y) annotation (Line(
      points={{428.52,75.7},{454,75.7},{454,132},{377.3,132},{377.3,165}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(Temp_ORC_cond_outlet.port, ORCunitHex.Outletcond) annotation (Line(
      points={{24,78},{-2,78},{-2,46},{-27.9,46},{-27.9,24}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(Temp_out_chiller.T, hysteresis.u) annotation (Line(
      points={{285.4,86},{292,86},{292,210},{316,210},{316,212}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(pID_HEX.CS, sourceMdot.in_T) annotation (Line(
      points={{35.57,189.5},{105.8,189.5},{105.8,150}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(Qeva1.y, sourceMdot.in_Mdot) annotation (Line(
      points={{58.5,149},{82,149},{82,158},{100,158},{100,150}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(Temp_in_chiller.port, HxOilWater.OutFlowPs) annotation (Line(
      points={{192,172},{192,132.8},{207.54,132.8}},
      color={0,127,255},
      smooth=Smooth.None));
