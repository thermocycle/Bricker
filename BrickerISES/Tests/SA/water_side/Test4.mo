...
model Test4
  ...
  Modelica.Blocks.Sources.Constant Qeva2(k=-387000)
    annotation (Placement(transformation(extent={{-5,-5},{5,5}},
        rotation=-90,
        origin={357,119})));
  Modelica.Blocks.Sources.Constant Qeva3(k=60 + 273)
    annotation (Placement(transformation(extent={{-5,-5},{5,5}},
        rotation=-90,
        origin={389,83})));
  ...
  Modelica.Blocks.Sources.Constant const1(k=68.89 + 273)
    annotation (Placement(transformation(extent={{-5,-5},{5,5}},
        rotation=0,
        origin={-25,191})));
  ThermoCycle.Components.Units.ControlSystems.PID pID_coolingT(
    PVmin=0,
    PVmax=440,
    Td=0,
    Ti=0.2,
    ...
    Kp=0.1,
    CSmax=-1,
    CSstart=-1) annotation (Placement(transformation(
        extent={{-9.5,7.5},{9.5,-7.5}},
        rotation=180,
        origin={365.5,47.5})));
  ...
  Modelica.Blocks.Logical.Hysteresis hysteresis(uLow=0, uHigh=1)
    annotation (Placement(transformation(extent={{-134,190},{-114,210}})));
  Modelica.Blocks.Logical.Not not1
    annotation (Placement(transformation(extent={{-136,154},{-116,174}})));
  Modelica.Blocks.Math.BooleanToReal booleanToReal
    annotation (Placement(transformation(extent={{-68,142},{-48,162}})));
equation
  ...
  connect(Cooling_tower.outlet_fl2, Ad_chiller.outlet_fl2) annotation (Line(
      ...
      color={0,0,255},
      smooth=Smooth.None));
  connect(Cooling_tower.inlet_fl2, Mflow_cond.port_a) annotation (Line(
  ...
  connect(const1.y, pID_HEX.SP) annotation (Line(
      points={{-19.5,191},{-3.75,191},{-3.75,190.5},{12,190.5}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(pID_HEX.PV, Temp_in_chiller.T) annotation (Line(
      points={{12,184.5},{6,184.5},{6,184},{4,184},{4,168},{166,168},{166,184},
          {179.4,184}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(pID_coolingT.PV, Temp_out_cooling_tower.T) annotation (Line(
      points={{375,44.5},{382,44.5},{382,22},{358,22},{358,2},{365.4,2}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(pID_coolingT.SP, Qeva3.y) annotation (Line(
      points={{375,50.5},{389,50.5},{389,77.5}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(pID_coolingT.CS, Cooling_tower.u) annotation (Line(
      points={{355.43,47.5},{346,47.5},{346,29},{333.2,29}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(pID_HEX.CS, sourceMdot.in_Mdot) annotation (Line(
      points={{31.57,187.5},{100,187.5},{100,150}},
      color={0,0,127},
      smooth=Smooth.None));
