...
model Test_SF_BiomassSystem_ORC_Hx_II_Simone_V3_2_prova
...
ThermoCycle.Components.Units.Solar.SolarField_Soltigua_Inc        solarCollector_Soltigua(
  ...
  ThermoCycle.Components.Units.ExpansionAndCompressionMachines.Pump pump3(
    ...
    hstart=145528,
    p_su_start=350000)
    annotation (Placement(transformation(extent={{158,124},{178,144}})));
 ...
 BrickerISES.Components.Valve_lin ValveSF1(
  ...
  Modelica.Fluid.Sensors.Temperature Temp_HEX_water_inlet(redeclare package
    ...
    annotation (Placement(transformation(extent={{254,94},{232,108}})));
 BrickerISES.Components.Valve_lin ValveSF2(
  ...
  Modelica.Fluid.Sensors.Temperature Temp_HEX_water_outlet(redeclare package
      Medium = ThermoCycle.Media.Water)
    annotation (Placement(transformation(extent={{252,138},{232,124}})));
  ...
  ThermoCycle.Components.FluidFlow.Reservoirs.SourceMdot sourceMdot1(
    ...
    Mdot_0=15,
    p=100000,
    T_0=323.15) annotation (Placement(transformation(
        extent={{-10,10},{10,-10}},
        rotation=90,
        origin={366,48})));
  ...
  Modelica.Fluid.Sensors.Temperature Temp_HEX_water_inlet1(redeclare package
      Medium = ThermoCycle.Media.Water)
    annotation (Placement(transformation(extent={{342,112},{324,124}})));
  ThermoCycle.Components.Units.Tanks.OpenTank Tank2(
    ...
    Tstart=477.15,
    pstart=450000)
    annotation (Placement(transformation(extent={{306,50},{326,70}})));
  ...
  Modelica.Blocks.Sources.Constant const7(k=333)
    annotation (Placement(transformation(extent={{288,-62},{298,-52}})));
equation
  ...
  connect(DpSF2.InFlow, Mflow_Bm_out.port_b) annotation (Line(
      ...
      color={0,0,255},
      smooth=Smooth.None));
  connect(massFlowRate_mix.port_b, ValveSF1.InFlow) annotation (Line(
      points={{81,41},{81,65.5},{62,65.5},{62,91}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(massFlowRate_mix.port_b, oRCunitHx.InletSf) annotation (Line(
      points={{81,41},{90,41},{90,-26},{128,-26},{128,-27.3333},{165.143,
          -27.3333}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(DpSF3.OutFlow, massFlowRate_mix.port_a) annotation (Line(
      points={{47.4,42},{56,42},{56,41},{67,41}},
      color={0,0,255},
      smooth=Smooth.None));
  ...
  connect(Inlet_orc.port, massFlowRate_mix.port_a) annotation (Line(
      points={{48,56},{50,56},{50,50},{56,50},{56,41},{67,41}},
      color={0,127,255},
      smooth=Smooth.None));
  ...
  connect(pID2.CS, ValveSF1.cmd) annotation (Line(
      points={{32.3,102},{44,102},{44,100},{54,100}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(Temp_HEX_water_inlet.port, hx1DInc1.inlet_fl1) annotation (Line(
      points={{243,94},{243,90},{204.923,90},{204.923,97.3462}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(sourceMdot1.flangeB, Mflow_HEX_water1.port_a) annotation (Line(
      points={{366,57},{366,70}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(hx1DInc2.outlet_fl1, sinkP1.flangeB) annotation (Line(
      points={{364.923,129.654},{364.923,136},{384,136},{384,147.6}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(Mflow_HEX_water1.port_b, hx1DInc2.inlet_fl1) annotation (Line(
      points={{366,84},{364.923,84},{364.923,107.346}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(hx1DInc1.outlet_fl1, hx1DInc2.inlet_fl2) annotation (Line(
      points={{204.923,119.654},{204.923,146},{355.192,146},{355.192,129.431}},
      color={0,0,255},
      smooth=Smooth.None));

  connect(Temp_HEX_water_inlet1.port, hx1DInc2.outlet_fl2) annotation (Line(
      points={{333,112},{334,112},{334,102},{355.369,102},{355.369,107.569}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(Temp_HEX_water_outlet.port, hx1DInc2.inlet_fl2) annotation (Line(
      points={{242,138},{242,146},{355.192,146},{355.192,129.431}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(pID2.PV, Temp_HEX_water_inlet1.T) annotation (Line(
      points={{22,99.6},{16,99.6},{16,162},{308,162},{308,118},{326.7,118}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(Mflow_HEX_water.port_a, pump4.OutFlow) annotation (Line(
      points={{251,53},{264.5,53},{264.5,53.4},{276.4,53.4}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(Mflow_HEX_water.port_b, hx1DInc1.inlet_fl1) annotation (Line(
      points={{237,53},{238,53},{238,52},{238,52},{204.923,52},{204.923,97.3462}},
      color={0,127,255},
      smooth=Smooth.None));

  connect(Tank2.InFlow, pump4.InFlow) annotation (Line(
      points={{306.2,51.6},{289.2,51.6},{289.2,46.5}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(Tank2.OutFlow, hx1DInc2.outlet_fl2) annotation (Line(
      points={{325.8,51.6},{355.369,51.6},{355.369,107.569}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(const7.y, pID4.SP) annotation (Line(
      points={{298.5,-57},{330.25,-57},{330.25,14.4},{316,14.4}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(pID4.CS, pump4.flow_in) annotation (Line(
      points={{305.7,12},{296,12},{296,78},{285.2,78},{285.2,54}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(pID4.PV, Temp_HEX_water_inlet1.T) annotation (Line(
      points={{316,9.6},{316,24},{320,24},{320,38},{326.7,38},{326.7,118}},
      color={0,0,127},
      smooth=Smooth.None));
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-260,
            -200},{440,200}}), graphics), Icon(coordinateSystem(extent={{-260,
            -200},{440,200}})));
end Test_SF_BiomassSystem_ORC_Hx_II_Simone_V3_2_prova;
