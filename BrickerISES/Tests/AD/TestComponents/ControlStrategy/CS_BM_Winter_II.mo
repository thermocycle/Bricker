...
BrickerISES.Components.SF.SF_SoltiguaL_IncNoWall _SF(
  ...
  Modelica.Blocks.Sources.Constant ORCToZero(k=0)
    annotation (Placement(transformation(extent={{-276,76},{-266,86}})));
equation
//    /* Energy calculation */
...
//    eta_sys_CHP = eta_ORC_chp*eta_SF*eta_BM*eta_HX; //or (ORC.Pel_gross + ORC.Pth_CfRank+HxOilWater.Q_wf_)/(Q_BM +Q_SF) with Q_BM based on biomass burnt and Q_SF = DNI*A_eff*Theta
  ...
  connect(Boo_HXWater_Temp.y, PID_HX.SP) annotation (Line(
      ...
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(ORCToZero.y, _ORC.QevapORC) annotation (Line(
      points={{-265.5,81},{-229.867,81},{-229.867,38.6444}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(ORCToZero.y, _BD.QORC) annotation (Line(
      points={{-265.5,81},{-224,81},{-224,78},{-60,78},{-60,-40},{222,-40},{222,
          58},{208.24,58}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(ORCToZero.y, _ORC.ORCon) annotation (Line(
      points={{-265.5,81},{-250,81},{-250,0.3},{-241.533,0.3}},
      color={0,0,127},
      smooth=Smooth.None));
