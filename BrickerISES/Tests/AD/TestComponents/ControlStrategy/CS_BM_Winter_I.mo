...
BrickerISES.Components.SF.SF_SoltiguaL_IncNoWall _SF(
  ...
  Modelica.Blocks.Math.BooleanToReal Boo_EvaORC(realTrue=-472000, realFalse=0)
    annotation (Placement(transformation(extent={{-672,58},{-658,72}})));
  Components.SourceComp.BrickerSource brickerSource(redeclare package
      AmbientDataTable =
        BrickerISES.Components.SourceComp.Tables.Winter_0101_0701)
  ...
  Modelica.Blocks.Math.BooleanToReal Boo_CondORC(realTrue=387000, realFalse=0)
    annotation (Placement(transformation(extent={{-672,32},{-658,46}})));
  Modelica.Blocks.Math.BooleanToReal Boo_ORCon(realTrue=1, realFalse=0)
    annotation (Placement(transformation(extent={{-672,10},{-658,24}})));
  Modelica.Blocks.Logical.Hysteresis HystQbuild(uLow=-387000, uHigh=-385000)
    annotation (Placement(transformation(extent={{-766,76},{-752,90}})));
  Modelica.Blocks.Math.BooleanToReal Boo_HXWater_Temp(realFalse=70 + 273.15,
      realTrue=60 + 273.15)
    annotation (Placement(transformation(extent={{-722,80},{-708,94}})));
  ...
  Modelica.Blocks.Logical.Hysteresis HystHours(
    uLow=7,
    uHigh=20,
    pre_y_start=false)
    annotation (Placement(transformation(extent={{-752,22},{-738,36}})));
  Modelica.Blocks.Logical.Not not2
    annotation (Placement(transformation(extent={{-726,22},{-716,32}})));
  Modelica.Blocks.Logical.And and2
    annotation (Placement(transformation(extent={{-708,6},{-694,20}})));
  Modelica.Blocks.Logical.LessThreshold lessThreshold(threshold=7)
    annotation (Placement(transformation(extent={{-750,-2},{-738,10}})));
  Modelica.Blocks.Logical.Not not1
    annotation (Placement(transformation(extent={{-730,-2},{-720,8}})));
equation
//    /* Energy calculation */
...
//    eta_sys_CHP = eta_ORC_chp*eta_SF*eta_BM*eta_HX; //or (ORC.Pel_gross + ORC.Pth_CfRank+HxOilWater.Q_wf_)/(Q_BM +Q_SF) with Q_BM based on biomass burnt and Q_SF = DNI*A_eff*Theta
  ...
  connect(Boo_EvaORC.y, _ORC.QevapORC) annotation (Line(
      points={{-657.3,65},{-606,65},{-606,32},{-292,32},{-292,50},{-229.867,50},
          {-229.867,38.6444}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  ...
  connect(Boo_CondORC.y, _ORC.QcondORC) annotation (Line(
      points={{-657.3,39},{-270,39},{-270,54},{-210.733,54},{-210.733,38.9667}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  ...
  connect(Boo_ORCon.y, _ORC.ORCon) annotation (Line(
      points={{-657.3,17},{-482,17},{-482,26},{-294,26},{-294,0.3},{-241.533,
          0.3}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(HystQbuild.u, _BD.Qbuilding) annotation (Line(
      points={{-767.4,83},{-768,83},{-768,-142},{186,-142},{186,78},{208.52,78},
          {208.52,81.7}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(HystQbuild.y, Boo_HXWater_Temp.u) annotation (Line(
      points={{-751.3,83},{-742,83},{-742,87},{-723.4,87}},
      color={255,0,255},
      smooth=Smooth.None));
  ...
  connect(Boo_HXWater_Temp.y, PID_HX.SP) annotation (Line(
      points={{-707.3,87},{-646,87},{-646,122},{-704,122},{-704,196},{-396,196},
          {-396,181.4}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(Boo_ORCon.u, Boo_CondORC.u) annotation (Line(
      points={{-673.4,17},{-680,17},{-680,39},{-673.4,39}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(lessThreshold.y, not1.u) annotation (Line(
      points={{-737.4,4},{-736,4},{-736,3},{-731,3}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(HystHours.y, not2.u) annotation (Line(
      points={{-737.3,29},{-731.65,29},{-731.65,27},{-727,27}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(not2.y, and2.u1) annotation (Line(
      points={{-715.5,27},{-715.5,19.5},{-709.4,19.5},{-709.4,13}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(not1.y, and2.u2) annotation (Line(
      points={{-719.5,3},{-714.75,3},{-714.75,7.4},{-709.4,7.4}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(lessThreshold.u, brickerSource.Hours) annotation (Line(
      points={{-751.2,4},{-760,4},{-760,-102},{-633.8,-102},{-633.8,-90.4}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(HystHours.u, brickerSource.Hours) annotation (Line(
      points={{-753.4,29},{-760,29},{-760,-102},{-633.8,-102},{-633.8,-90.4}},
      color={0,0,127},
      smooth=Smooth.None,
      pattern=LinePattern.Dash));
  connect(and2.y, Boo_EvaORC.u) annotation (Line(
      points={{-693.3,13},{-688,13},{-688,65},{-673.4,65}},
      color={255,0,255},
      smooth=Smooth.None));
  connect(and2.y, Boo_CondORC.u) annotation (Line(
      points={{-693.3,13},{-684,13},{-684,40},{-680,40},{-680,39},{-673.4,39}},
  ...
  annotation (Diagram(coordinateSystem(preserveAspectRatio=false, extent={{-780,
            ...
            -200},{260,340}})),
    experiment(StartTime=-5000, StopTime=259200),
