...
model SFDefDynBMConstHx_P_All_II
 ...
 BrickerISES.Components.Valve_lin ValveSF(
  ...
  Modelica.Fluid.Sensors.Temperature Temp_HEX_oil_inlet(redeclare package
      Medium =
        ThermoCycle.Media.Incompressible.IncompressibleTables.TherminolSP)
    annotation (Placement(transformation(extent={{-278,198},{-250,216}})));
...
BrickerISES.Components.SF.SF_SoltiguaL_IncNoWall SF(
  ...
  ThermoCycle.Components.Units.ExpansionAndCompressionMachines.Pump_Mdot
    pump_Mdot3(
        ...
        ThermoCycle.Media.Incompressible.IncompressibleTables.TherminolSP,
      Mdot_0=2.7)
    annotation (Placement(transformation(extent={{-332,174},{-312,194}})));
  Components.GeneralCombiTableModel.GeneralCombiTable generalCombiTable(
    ...
    annotation (Placement(transformation(extent={{-664,-92},{-644,-72}})));
equation
//    /* Energy calculation */
...
//    eta_sys_CHP = eta_ORC_chp*eta_SF*eta_BM*eta_HX; //or (ORC.Pel_gross + ORC.Pth_CfRank+HxOilWater.Q_wf_)/(Q_BM +Q_SF) with Q_BM based on biomass burnt and Q_SF = DNI*A_eff*Theta
  ...
  connect(TempSfOut.T, hysteresis.u) annotation (Line(
      points={{-595.3,-2},{-624,-2},{-624,-120},{-613.2,-120}},
      color={0,0,127},
      smooth=Smooth.None,
  ...
  connect(Temp_out_chiller.port, Cooling_tower.outlet_fl2) annotation (Line(
      points={{131,108},{130,108},{130,96},{114,96},{114,98},{110,98},{109.76,
          83.6}},
      color={0,127,255},
      smooth=Smooth.None));
  ...
  connect(Temp_HEX_water_inlet.port, pump_Mdot2.outlet) annotation (Line(
      ...
      color={0,127,255},
      smooth=Smooth.None));
  connect(pump_Mdot3.inlet, MflowBM_mix.port_b) annotation (Line(
      points={{-329.2,184.5},{-340,184.5},{-340,124},{-434,124}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(pump_Mdot3.outlet, HxOilWater.InFlowSs) annotation (Line(
      points={{-316.4,191.4},{-242,191.4},{-242,191.2},{-226.56,191.2}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(pID2.CS, pump_Mdot3.flow_in) annotation (Line(
      points={{-379.52,177},{-366,177},{-366,192},{-325.2,192}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(Temp_HEX_oil_inlet.port, HxOilWater.InFlowSs) annotation (Line(
      points={{-264,198},{-264,192},{-242,191.4},{-242,191.2},{-226.56,191.2}},
      color={0,127,255},
      smooth=Smooth.None));
...
end SFDefDynBMConstHx_P_All_II;
