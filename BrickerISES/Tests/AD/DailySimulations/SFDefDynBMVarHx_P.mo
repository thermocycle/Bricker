...
model SFDefDynBMVarHx_P
/* Simulazione di una giornata tipo - carico termico variabile - Def parziale */
 ...
 BrickerISES.Components.Valve_lin ValveSF(
  ...
  Components.GeneralCombiTableModel.GeneralCombiTable generalCombiTable(
    ...
    annotation (Placement(transformation(extent={{288,44},{274,58}})));
...
equation
  ...
  connect(HxOilWater.outlet_fl1, sinkP.flangeB) annotation (Line(
      points={{204.923,117.654},{204.923,134},{306,134},{306,137.6}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(HxOilWater.outlet_fl2, MflowHXl.port_a) annotation (Line(
      points={{195.369,95.5692},{194,95.5692},{194,58},{107,58},{107,-64}},
      color={0,0,255},
      smooth=Smooth.None));
  connect(PumpHxOil_2.OutFlow, HxOilWater.inlet_fl2) annotation (Line(
      points={{179.4,152.1},{195.192,152.1},{195.192,117.431}},
      color={0,0,255},
      smooth=Smooth.None));
  ...
  connect(Mflow_HEX_water.port_b, HxOilWater.inlet_fl1) annotation (Line(
      points={{226,82},{226,90},{204.923,90},{204.923,95.3462}},
      color={0,127,255},
      smooth=Smooth.None));
  connect(Temp_HEX_water_inlet.port, HxOilWater.inlet_fl1) annotation (Line(
      points={{261,94},{261,90},{204.923,90},{204.923,95.3462}},
      color={0,127,255},
      smooth=Smooth.None));
  ...
  connect(Temp_HEX_oil_outlet.port, HxOilWater.outlet_fl2) annotation (Line(
      points={{157,28},{194,28},{194,95.5692},{195.369,95.5692}},
      color={0,127,255},
      smooth=Smooth.None));
