 ...
 BrickerISES.Components.Valve_lin ValveSF(
  ...
  Modelica.Fluid.Sensors.MassFlowRate MflowSF_Recirculation(redeclare package
    ...
    annotation (Placement(transformation(
        ...
        rotation=90,
        origin={-70,-120})));
  Modelica.Blocks.Logical.Hysteresis hysteresis(uLow=210 + 273.15, uHigh=228 +
        273.15)
...
ThermoCycle.Components.Units.Solar.SolarField_Soltigua_Inc SF(
  ...
  Modelica.Blocks.Sources.Trapezoid DNI(
    ...
    amplitude=-650,
    startTime=5000)
    annotation (Placement(transformation(extent={{-200,-150},{-190,-140}})));
  ThermoCycle.Functions.Init init(
    t_init=2000,
    length=20,
    noevent=true)
    annotation (Placement(transformation(extent={{-198,26},{-190,34}})));
  Modelica.Blocks.Sources.Constant ConstantInput(k=0.44)
    annotation (Placement(transformation(extent={{-230,12},{-220,22}})));
equation
  ...
  connect(hysteresis.y, SF.Defocusing) annotation (Line(
      points={{-181.4,-172},{-166,-172},{-166,-139.818},{-151.667,-139.818}},
      color={255,0,255},
      smooth=Smooth.None));
  ...
  connect(DNI.y, SF.DNI) annotation (Line(
      points={{-189.5,-145},{-178,-145},{-178,-126.136},{-151.9,-126.136}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(pID1.CS, init.u2) annotation (Line(
      points={{-207.7,42},{-204,42},{-204,31.6},{-198.8,31.6}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(init.y, BM.u) annotation (Line(
      points={{-189.68,30.08},{-184,30.08},{-184,22},{-171.8,22}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(ConstantInput.y, init.u1) annotation (Line(
      points={{-219.5,17},{-212,17},{-212,28.48},{-198.8,28.48}},
      color={0,0,127},
      smooth=Smooth.None));
