...
model HEX "Model of HEX simplified"
...
/******************************* COMPONENTS ***********************************/
  ...
  BrickerISES.Components.Source_Q_L source_Q(A=Aext)
...
protected
  ...
  SummaryClass Summary( T_profile( Twf = WorkingFluid.T), p_wf= WorkingFluid.p,Q_wf = Q_wf_);
...
der(E_HX) = Q_wf_;
  ...
  connect(u, source_Q.Q_tot) annotation (Line(
      points={{0,-104},{0,-23.6}},
      color={0,0,127},
      smooth=Smooth.None));
  connect(source_Q.thermalPort, WorkingFluid.Wall_int) annotation (Line(
      points={{-0.24,-4.16},{-0.24,25.92},{0,25.92},{0,57.5}},
      color={255,0,0},
      smooth=Smooth.None));
  annotation (
