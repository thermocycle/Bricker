...
model ValveControl_SecondVersion
  "Valve with incompressible flow hypothesis for the calculation of the pressure drop and an embedded controller based on an external input"
  ...
  parameter Real Var_Max "Maximum value for ExternalInputs";
  parameter Real Var_Min "Minimum value for ExternalInputs";
  parameter Real Length "Length for the weighting factor";
  ...
  Real cmd "Total valve fraction open";
  Real cmd_ExternalInput "Valve fraction open based on ExternalInput";
  ThermoCycle.Interfaces.Fluid.FlangeA InFlow(redeclare package Medium =
        Medium) annotation (Placement(transformation(extent={{-100,-10},
            {-80,10}})));
  ThermoCycle.Interfaces.Fluid.FlangeB OutFlow(redeclare package Medium =
        Medium)
    annotation (Placement(transformation(extent={{80,-10},{100,10}})));
  Modelica.Blocks.Interfaces.RealInput ExternalInput annotation (Placement(
        transformation(
        origin={-18,80},
        extent={{-20,-20},{20,20}},
...
equation
  ...
  // Open valve when DNI < triggerVal_DNI and T_tank > triggerVal_Ttank
  cmd_ExternalInput = BrickerISES.Functions.WeightingFactor(
    t_max=Var_Max,
    t_min=Var_Min,
    length=Length,
    Var=ExternalInput);

  cmd = cmd_ExternalInput;
  A = Afull*cmd;
  if cardinality(ExternalInput) == 0 then
    cmd = Xopen;
  end if;
  Mdot = A*sqrt(2*rho)*smooth(1, noEvent(if (DELTAp_bis > DELTAp_0)
     then sqrt(DELTAp_bis) else if (DELTAp_bis < -DELTAp_0) then -sqrt(
    -DELTAp_bis) else sqrt(DELTAp_0)*(DELTAp_bis/DELTAp_0)/4*(5 - (
    DELTAp_bis/DELTAp_0)^2)));
  // end if;
  // Boundary conditions
  Mdot = InFlow.m_flow;
...
initial equation
  ...
  annotation (
    Icon(coordinateSystem(preserveAspectRatio=true,  extent={{-100,-100},{100,100}}),
                         graphics={Text(extent={{-100,-40},{100,-74}},
          textString="%name")}),
...
end ValveControl_SecondVersion;
